...
class WriterNaive:
    ...
    def draw_line_thick(self, x1, y1, x2, y2, width, color) -> None:
        ...
        thickLineColor(self.renderer, int(x1), int(self.ySize-y1), int(x2), int(self.ySize-y2),int(width), color)
    ...
    def draw_rectangle(self, x1, y1, width, height, color, filled:bool) -> None:
        ...
        if filled:
            boxColor(self.renderer,int(x1),int(self.ySize-y1),int(x1+width),int((self.ySize-(y1+height))),color)
        else:
            rectangleColor(self.renderer, int(x1), int(self.ySize-y1), int(x1+width),int((self.ySize-(y1+height))), color)
    ...
    def draw_circle(self, x, y, radius, color, filled:bool) -> None:
        ...
        if filled:
            filledCircleColor(self.renderer,int(x), int(self.ySize - y), int(radius), color)
        else:
            aacircleColor(self.renderer, int(x), int(self.ySize - y), int(radius), color)
    ...
    def draw_polygon(self, x, y, radius, n, color, filled:bool) -> None:
        ...
        for i in range(n):
            ...
            ys.append(int((self.ySize)-y+math.sin((rads*i)-(math.pi/2))*radius))
