...
FrameResult = Image.Image | bool | None
FrameCallback = Callable[[Canvas, Writer, int], FrameResult]
...
def loop(
    ...
    Args:
        ...
        x_size (int, optional): The **width** of the rendering window in pixels. Defaults to 300.
        y_size (int, optional): The **height** of the rendering window in pixels. Defaults to 300.
    ...
    def draw_animation(screen: Canvas, pen: Writer, frame_index: int):
        ...
        pen.draw_circle(x_size // 2, i, 5, Color.red, True)
    ...
    try:
        from IPython.display import clear_output, display  # noqa: PLC0415 - optional dependency
    except ImportError as exc:
        raise RuntimeError("Notebook embedding requires the 'interactive' project extra.") from exc

    screen = Canvas("Jupyter_Internal", x_size, y_size)
    pen = Writer(screen)
    ...
    def decorator(func: FrameCallback) -> None:
        completed = False
        try:
            for i in range(frame_count):
                ...
                if embedded:
                    if isinstance(im, Image.Image):
                        ...
                        if movie_writer and movie_writer.is_recording and movie_writer.frame_count() == frames_before:
            ...
            if movie_writer:
                ...
                print(f"Animation saved to: {output_path}")  # noqa: T201 - user-facing notebook status
            completed = True
        finally:
