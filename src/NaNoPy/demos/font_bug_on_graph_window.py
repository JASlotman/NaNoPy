...
class Cell:
    def __init__(self, row: int, column: int, quorum: bool) -> None:
        self.color_states = {"healthy": healthy_color, "starving": starving_color, "dead": dead_color, "repolarization": repolarization_color}
        ...
        self.nutrients_taken_up: set[Nutrient] = set()
        self.quorum = quorum

        self.calcium_ions: list[Calcium] = []
    ...
    def degrade_nutrient(self) -> None:
        for nutrient in tuple(self.nutrients_taken_up):
    ...
    def take_up_nutrient(self, nutrient: Nutrient) -> None:
    ...
    def calcium_induced_release(self, calc_ion: Calcium) -> None:
        (xmincell, ymincell, xmaxcell, ymaxcell) = self.area
        if (
            xmincell <= calc_ion.xpos <= xmaxcell
            and ymincell <= calc_ion.ypos <= ymaxcell
            and calc_ion not in self.calcium_ions
            and self.state != "dead"
            and self.polarization != "repolarization"
            and self.depolarization_timer > depolarization_time
            and self.quorum is True
        ):
            for calcium_ion in self.calcium_ions:
                calcium_ion.release()
    ...
    def nutrient_induced_release(self) -> None:
...
class Nutrient:
    def __init__(self, radius: float, speed: int, flow: int) -> None:
        self.speed = speed
        self.flow = flow
        self.radius = radius
    ...
    def take_up(self, cells: list[Cell]) -> None:
        for cell in cells:
            (xmincell, ymincell, xmaxcell, ymaxcell) = cell.area
            if xmincell <= self.xpos <= xmaxcell and ymincell <= self.ypos <= ymaxcell and not cell.dead:
...
def create_cells(rows: int, columns: int, percentage_quorum_sensing: float) -> None:
    ...
    while current_amount < amount_quorum_cells:
        ...
        if not cells[n].quorum:
            cells[n].quorum = True
            current_amount += 1


def fill_cells_calcium() -> None:
    for cell in cells:
        (xmincell, ymincell, xmaxcell, ymaxcell) = cell.area
...
def initialize(rows: int, columns: int, number_nutrients: int) -> None:
...
def create_graphs(height_graph: float, origin_x: float, origin_y: float, width_bar: float) -> None:
